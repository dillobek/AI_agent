...
13. [The AI agent & tool calling](#13-the-ai-agent--tool-calling)
    - [13.1 Real-time voice assistant (Gemini Live)](#131-real-time-voice-assistant-gemini-live)
...
> **Real-time Jarvis / n8n status:** The backend already provides Gemini Live ephemeral tokens,
> voice tool relay endpoints, and secured n8n inbound/outbound webhooks. The dashboard-side Live
> WebSocket/microphone client is still required for a complete real-time voice experience. See
> [docs/n8n-real-time-jarvis.md](docs/n8n-real-time-jarvis.md) for the exact contract and release checklist.
...
| Module toggles | `TELEGRAM_ENABLED`, `GOOGLE_DRIVE_ENABLED`, `OBSIDIAN_ENABLED`, `RAG_ENABLED`, `N8N_ENABLED`, `FINANCE_MODULE_ENABLED`, `PATIENTS_MODULE_ENABLED`, `DASHBOARD_ENABLED`, `VOICE_ENABLED`, `YOUTUBE_ENABLED`, `CALENDAR_ENABLED` | Each unlocks conditional required fields below |
...
| Voice (Gemini Live) | `GEMINI_LIVE_MODEL`, `VOICE_LIVE_TOKEN_TTL_SECONDS` | Reuses `GEMINI_API_KEY`; that key needs Live API access, see 13.1 |
| YouTube | `YOUTUBE_API_KEY` | Required only if `YOUTUBE_ENABLED=true` |
| Google Calendar | `GOOGLE_CALENDAR_ID` | Required only if `CALENDAR_ENABLED=true`; reuses `GOOGLE_APPLICATION_CREDENTIALS` |
...
| Voice assistant | `VOICE_ENABLED` | off | `/voice/live-token` and `/voice/execute-tool` return 503; no real-time "Salom Ali" voice UI (see 13.1) |
| YouTube search | `YOUTUBE_ENABLED` | off | Agent's `search_youtube` tool is not offered |
| Google Calendar | `CALENDAR_ENABLED` | off | `get_today_plan` still works (PlanItem + Obsidian only), just without Calendar events merged in |
...
4. Inbound: `N8N_INBOUND_SECRET` (generated by the setup wizard) must be sent as
   ...
   (`agent_prompt`, `finance_summary`, `patient_history`, `get_today_plan`) — n8n can never run
   arbitrary code or reach the filesystem through this endpoint.
5. Voice-triggered outbound events (once `VOICE_ENABLED=true`, see section 13.1): the same
   fire-and-forget `N8nService.notifyEvent(event, payload)` used by Finance/Patients also fires
   `voice.session_started` (when a Live session's ephemeral token is minted), `plan.item_created`
   (a `PlanItem` created via voice or the dashboard), and `voice.report_requested` (the
   `get_today_report` tool was called). Point an n8n Webhook node at these to, for example, push a
   Telegram notification whenever a plan item is created by voice — build that workflow in n8n
   itself; this app only guarantees the event fires with a stable payload shape.
...
### 13.1 Real-time voice assistant (Gemini Live)

Set `VOICE_ENABLED=true` (needs `GEMINI_API_KEY` with **Live API** access — a separate
capability from the standard `generateContent` calls the text agent uses; enable it for your
key in Google AI Studio / Vertex before relying on this) to turn on `POST /voice/live-token`
and `POST /voice/execute-tool` (`src/voice/`). The voice session itself is **not** proxied
through this backend:

1. The dashboard (M6, frontend) calls `POST /voice/live-token` and gets back a short-lived,
   single-use Gemini Live ephemeral token (`GoogleGenAI.authTokens.create`), locked to
   `GEMINI_LIVE_MODEL` and audio-only responses — a leaked token has a small blast radius
   compared to the real `GEMINI_API_KEY`, since it can't be used past its
   `VOICE_LIVE_TOKEN_TTL_SECONDS` expiry, for a different model, or for another session.
   Minting is audit-logged (`AuditLogService`) the same way login is.
2. The browser then opens its **own** WebSocket directly to Gemini Live using that token —
   audio in, audio out, no round trip through this API for the speech itself.
3. When Gemini Live decides to call a tool (e.g. `search_youtube`, `get_today_plan`), the
   frontend relays that single function call to `POST /voice/execute-tool`. This backend never
   re-enters `AgentService`'s reasoning loop for it — Live already decided which tool and with
   what arguments — it just runs `AgentToolsService.execute()` and returns the result. Calls are
   deduped by Gemini Live's own per-call id for a few minutes, since Live can retry a call across
   a WebSocket reconnect and some tools have side effects (creating a `PlanItem`, etc).
4. Both this endpoint and the text agent's tool-calling loop go through the same
   `ToolExecutionService` (`src/ai/tools/tool-execution.service.ts`) for timeout enforcement,
   output truncation, and `ExecutionLogService` logging — voice-triggered calls are tagged
   `voice-agent:<channelKey>` in the execution log, text-chat calls stay `ai-agent:<channelKey>`,
   so the two are easy to tell apart without losing either one's audit trail.

Both endpoints sit behind the same guard stack as every other authenticated route
(`ModuleEnabledGuard` → `JwtAuthGuard` → `RolesGuard`) and a rate limit well above login's —
a single spoken turn can legitimately trigger several function calls, or a token refresh, in
quick succession.

`YOUTUBE_ENABLED`/`CALENDAR_ENABLED` are separate flags (not folded into `VOICE_ENABLED`)
because `search_youtube`/`get_today_plan` are also useful from the existing text-chat agent —
see section 7 for the full flag list.

Not built yet, tracked as follow-on work: the frontend mic-capture/playback UI itself (M6),
the "Salom Ali" wake-word listener (M7, a client-side TensorFlow.js model you train yourself on
your own voice via Google's Teachable Machine — no commercial wake-word engine covers Uzbek),
and the YouTube embed panel (M8).
